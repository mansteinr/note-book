...
实时数据接入层使用 Kafka 进行消息缓冲和削峰，Flink 负责分布式实时计算，ClickHouse 承担海量数据的 OLAP 查询，Java 微服务通过多实例部署和负载均衡进行横向扩展。

PCDN 模块本身主要是查询已经经过 Flink 聚合处理后的 ClickHouse 数据，而不是直接处理海量原始 NetFlow，因此压力主要集中在 ClickHouse 查询层。我们通过预聚合、时间范围限制、分页和减少无效查询来降低压力。
...
Partition 1
```
如果节点ID不变，那么消息就会一直进入 Partition 1。同一个 nodeId 的消息可以尽量进入同一个 Partition。
...
是数据库事务的四个核心特性，用来保证数据在发生各种异常（比如系统崩溃、并发操作）时，依然是正确和可靠的。

A (Atomicity) —— 原子性：事务中的所有操作，要么全部成功，要么全部失败。
    经典场景：转账操作，A账户减100元，B账户加100元。这两步必须看成一个整体。如果扣钱成功但加钱失败，整个事务就会回滚，A账户的钱也会恢复，不会出现“钱转丢了”的情况。

    实现机制：主要依赖数据库的 Undo Log（回滚日志）。当事务需要回滚时，数据库就利用这些日志，将数据恢复到事务开始前的样子。

C (Consistency) —— 一致性：事务必须让数据库从一个正确的状态，转变为另一个正确的状态。

    经典场景：银行规定，所有账户余额总和不能为负。一个事务完成后，数据库必须仍然满足这个约束。如果事务逻辑本身有Bug，试图把余额变成负数，即使其他三个特性都满足，一致性也会被破坏。

    实现机制：这需要数据库的约束（如外键、唯一索引）和应用程序的正确逻辑共同保证。数据库只能辅助，最终要靠开发者写对SQL。

I (Isolation) —— 隔离性：多个事务并发执行时，彼此的操作不会互相干扰。
    并发问题：如果不考虑隔离性，会出现三种经典问题：

    脏读 (Dirty Read)：读到另一个事务未提交的数据（可能被回滚）。

    不可重复读 (Non-Repeatable Read)：在同一个事务内，两次读同一行数据，结果不一样（因为其他事务修改并提交了）。

    幻读 (Phantom Read)：在同一个事务内，两次查询符合条件的行数不一样（因为其他事务插入或删除了数据）。
    SQL标准定义了四种隔离级别，级别越高，性能开销越大：
        读未提交 (Read Uncommitted) 
        读已提交 (Read Committed)
        可重复读 (Repeatable Read)
        串行化 (Serializable)
D (Durability) —— 持久性：事务一旦提交，它对数据库的修改就是永久性的。
    经典场景：你转账成功并收到银行短信后，哪怕下一秒机房断电、硬盘损坏，这笔记录也必须存在（除非硬盘物理损毁）。

    实现机制：主要依赖 Redo Log（重做日志）。事务提交时，修改会先写入Redo Log，即使数据页还没来得及写入磁盘，数据库重启后也能通过Redo Log恢复已提交的事务。