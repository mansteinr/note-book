...
gantt
    ...
    4.3 验收:完整评估报告 + 回滚预案     :milestone, d3, after d2, 1d
```

### 12.3 验收清单(13 项,缺一不可)

- [x] **指标**:7 大类 40+ 指标全部对应代码函数,单测覆盖
- [x] **数据集**:2000 条金标准,分层难度分布符合 §4.2 配比 ±5%
- [x] **数据集**:PII 泄漏抽检 1000 条 = 0;双审 Kappa ≥ 0.85
- [x] **基准**:4 维基线矩阵(硬件/尺寸/精度/引擎)已校准并入库
- [x] **流程**:离线 5 步流水线可一条命令复现
- [x] **显著性**:模型间 Δ 必跑 bootstrap,结果含 p 值 + 95% CI
- [x] **质量可视化**:4 大看板(雷达/混淆/热力图/Pareto)可一键生成
- [x] **场景对比**:企业 QA / 代码 / 创意 / 抽取 4 场景分别跑过
- [x] **工程-速度**:TTFT / ITL / tokens/s / QPS / P50-P99 5 口径齐
- [x] **工程-资源**:GPU 显存 / GPU util / CPU / RSS 4 项齐,1s 粒度
- [x] **工程-稳定**:72h + 10 项故障注入脚本已跑,满足 §9.3 通过标准
- [x] **在线 A/B**:5% → 20% → 50% → 100% 闸门 + 自动回滚脚本
- [x] **合规**:评估全流程留痕可审计(数据卡/评估日志/决策记录/版本快照)

---

> **参考来源 & 标准化对齐**:
> - [HELM: Holistic Evaluation of Language Models (Stanford CRFM)](https://crfm.stanford.edu/helm/latest/) — 多场景多指标评估的业界标杆,本文 7 维体系对齐其层级拆分思想
> - [Open LLM Leaderboard (HuggingFace)](https://huggingface.co/spaces/open-llm-leaderboard/open_llm_leaderboard) — MMLU/ARC/HellaSwag/MATH/GSM8K 6 项通用基准指标口径对齐
> - [CMMLU / C-Eval](https://github.com/haoranxu/CMMLU) — 中文知识推理基准,§4.3 opensource 数据来源
> - [Garak: LLM Vulnerability Scanner](https://github.com/NVIDIA/garak) — §3.4 越狱/偏见/PII 自动化扫描工具选型
> - [Microsoft PromptBench / LLM Security Eval](https://github.com/microsoft/promptbench) — 对抗样本评估框架,对应 §4.2 Adversarial 5% 分层
> - [LM Evaluation Harness (EleutherAI)](https://github.com/EleutherAI/lm-evaluation-harness) — 200+ 任务统一评估框架,§10 `LLMEvaluator` 的任务扩展可直接封装其 `Task` 接口
> - [SWE-bench: Code 生成真实仓库级基准](https://www.swebench.com/) — §8.2 代码生成场景的工业级 Pass@k 金标准
> - [ISO/IEC 42001 AI 管理体系](https://www.iso.org/standard/81231.html) — §九 稳定性 & 安全评估的合规框架对齐
