...
│                                                     │
│  1. 模型初始化                                       │
│     ├── 权重随机初始化（Xavier/He 初始化）           │
│     ├── Embedding 层初始化                           │
│     └── 特殊: LayerNorm 权重初始化为 1，偏置为 0     │
│                                                     │
│  2. 数据加载                                         │
│     ├── 打包为固定长度序列（如 4096 tokens）          │
│     ├── 随机打乱                                     │
│     └── 动态 batch（前面用短序列，后期用长序列）      │
│                                                     │
│  3. 前向传播                                         │
│     ├── 输入 → Embedding → 位置编码                  │
│     ├── Transformer Layers (Self-Attention + FFN)    │
│     └── 输出层 → Logits → softmax → 概率分布         │
│                                                     │
│  4. 计算损失                                         │
│     └── Cross-Entropy Loss（预测 token vs 真实 token）│
│                                                     │
│  5. 反向传播                                         │
│     ├── 计算梯度                                     │
│     ├── 梯度裁剪（防止梯度爆炸，通常裁剪到 1.0）      │
│     └── 梯度累积（大 batch 模拟）                    │
│                                                     │
│  6. 参数更新                                         │
│     ├── 优化器: AdamW                                │
