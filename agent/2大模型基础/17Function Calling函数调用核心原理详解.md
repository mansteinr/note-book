...
以下是实现 Function Calling 的最简 5 步流程：

```mermaid
flowchart LR
    A[Step 1<br/>定义函数] --> B[Step 2<br/>注册函数]
    B --> C[Step 3<br/>发送请求]
    C --> D[Step 4<br/>执行函数]
    D --> E[Step 5<br/>返回结果]
```
...
| 问题 | 原因 | 解决方案 |
|------|------|---------|
| 模型不调用函数 | 函数描述不清晰，或问题不需要函数 | 优化 description，明确触发场景 |
| 函数名识别错误 | 函数名与注册名不匹配 | 检查拼写，使用小写加下划线命名 |
| 参数生成不完整 | 用户输入信息不足 | 在函数描述中说明参数要求，模型会主动追问 |
| 结果不被模型理解 | 返回格式不清晰 | 使用 JSON 格式，字段命名清晰 |
| 多轮对话失效 | 历史消息中缺少 tool 角色消息 | 确保每次函数调用都添加对应的 tool 消息 |

### 附录六：各平台 Function Calling 支持对比

| 平台 | 支持名称 | 文档链接 | 特殊说明 |
|------|---------|---------|---------|
| **OpenAI** | Function Calling | [链接](https://platform.openai.com/docs/guides/function-calling) | 最先推出，生态最完善 |
| **Anthropic** | Tool Use | [链接](https://docs.anthropic.com/claude/docs/tool-use) | 支持并行工具调用 |
| **Google** | Function Calling | [链接](https://ai.google.dev/docs/function_calling) | Google Cloud 支持 |
| **Mistral** | Function Calling | [链接](https://docs.mistral.ai/function-calling/) | 支持多个函数调用 |
| **阿里通义** | 函数调用 | [链接](https://help.aliyun.com/zh/model-studio/) | 国内主流平台 |
| **百度千帆** | 函数调用 | [链接](https://qianfan.cloud.baidu.com/) | 国内主流平台 |
| **腾讯混元** | 工具调用 | [链接](https://cloud.tencent.com/product/hunyuan) | 国内主流平台 |

### 附录七：Function Calling 演进时间线

| 时间 | 里程碑事件 | 说明 |
|------|-----------|------|
| **2023.06** | OpenAI 推出 Function Calling | 首个原生支持函数调用的大模型 API |
| **2023.08** | 多函数并行调用支持 | 支持一次调用多个函数 |
| **2023.11** | Anthropic 推出 Tool Use | Claude 系列支持工具调用 |
| **2024.01** | 国内平台陆续支持 | 阿里、百度、腾讯等相继推出 |
| **2024.04** | GPT-4o 支持 Function Calling | 新一代模型支持 |
| **2024.06** | 函数调用成本优化 | 函数结果不消耗 completion tokens |

---

> **文档结语**：Function Calling 是大模型从"被动响应"走向"主动行动"的关键技术，它为构建真正智能的 Agent 系统奠定了坚实基础。掌握这一核心机制，开发者可以将大模型的能力与业务系统深度融合，创造出前所未有的智能应用。随着技术的不断演进，Function Calling 必将在更多场景中发挥重要作用，成为 AI 应用开发不可或缺的核心能力之一。
