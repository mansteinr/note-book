...
B. reactive可以处理基本类型
C. ref的.value如果是对象，内部会调用reactive处理
D. ref和reactive没有任何关系

**答案：C**

**4. 关于Vue3依赖收集过程，以下说法错误的是？**
A. 依赖收集发生在get操作中
B. 使用WeakMap存储目标对象与依赖的映射
C. 每个属性对应一个Set存储副作用函数
D. 依赖收集完成后不会发生变化

**答案：D**

**5. Vue3响应式系统中，effect函数的执行时机是？**
A. 创建时立即执行一次
B. 只在依赖变化时执行
C. 创建时执行，依赖变化时也执行
D. 由开发者手动调用
...
**7. Vue3中以下哪个API不会触发响应式更新？**
A. reactive(obj).count = 1
B. ref(0).value = 2
C. Object.assign(reactiveObj, { a: 1 })
D. ref('').value = 'hello'

**答案：C**

**8. 关于Vue3的浅响应式API，错误的是？**
A. shallowReactive只代理对象第一层
B. shallowRef的.value变化会触发更新
C. shallowReactive比reactive性能更好
D. shallowRef内部对象也是深层代理的

**答案：D**

**9. Vue3中toRef和toRefs的作用是？**
A. 将响应式对象转换为普通对象
B. 保持与源对象的响应式连接
C. 创建新的独立响应式数据
D. 将ref转换为reactive
