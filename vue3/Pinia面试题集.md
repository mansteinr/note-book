...
#### 【高级】题目 2：如何将项目从 Vuex 迁移到 Pinia？请提供完整的迁移步骤
...
**3. 转换 Vuex Module 为 Pinia Store**
...
// 转换为 Pinia store: stores/user.js
import { defineStore } from 'pinia'
...
export default {
  computed: {
    ...mapState('user', ['users', 'currentUser', 'userCount', 'isLoggedIn'])
  },
  methods: {
...
export default {
  setup() {
    ...
    const { users, currentUser, userCount, isLoggedIn } = storeToRefs(userStore)
    
    // actions 可以直接解构
    const { fetchUsers, login } = userStore
    
    return {
      users, currentUser, userCount, isLoggedIn,
      fetchUsers, login
    }
  }
}
