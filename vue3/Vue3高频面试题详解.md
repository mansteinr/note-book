...
- [Vue3 高频面试题与详细回答](#vue3-高频面试题与详细回答)
  ...
  - [二、响应式原理](#二响应式原理)
    ...
    - [Q6. toRef 和 toRefs 的作用？](#q6-toref-和-torefs-的作用)
      - [toRef](#toref)
      - [toRefs](#torefs)
      - [应用场景：解构 props 保持响应式](#应用场景解构-props-保持响应式)
...
Vue 3 的列表 Diff 使用的是快速 Diff（Fast Diff）算法，而 Vue 2 使用的是经典的双端 Diff 算法。Vue 3 会先从头部和尾部进行同步比较，跳过前后相同的节点，缩小 Diff 范围；然后快速处理新增和删除节点。对于中间乱序的节点，会通过 key 建立索引映射，并使用最长递增子序列 LIS 找出不需要移动的节点，从而尽可能减少真实 DOM 的移动操作

```
Vue 2：双端 Diff

Vue 3：快速 Diff（头尾同步 + key 映射 + LIS）
```
```
Vue3 Diff 分两步：
1. 双端对比：从两端开始对比相同的节点
2. 中间乱序：用最长递增子序列（LIS）找出已经处于相对正确顺序、不需要移动的节点，从而减少 DOM 移动次数。
```
