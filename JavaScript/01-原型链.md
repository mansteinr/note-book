...
下图展示 `new Person('Tom', 25)` 执行时的**四个内部步骤**，以及最终在构造函数、实例、原型对象之间形成的**三角关系**与**原型链**：

```mermaid
flowchart TB
    subgraph S1["new 的四个内部步骤"]
        direction TB
        A1["① 创建空对象<br/>obj = Object.create(Constructor.prototype)<br/>→ 设置 obj.__proto__ = Constructor.prototype"]
        A2["② 执行构造函数<br/>Constructor.apply(obj, args)<br/>→ this 绑定到 obj，添加自身属性"]
        A3["③ 判断返回值<br/>返回对象 → 用该对象<br/>否则 → 用 obj"]
        A1 --> A2 --> A3
    end

    subgraph S2["new Person('Tom', 25) 最终形成的原型链关系"]
        Person["Person 构造函数"]
        PP["Person.prototype<br/>━━━━━━━━━━<br/>constructor: Person<br/>greet: ƒ"]
        P["p 实例<br/>━━━━━━━━━━<br/>name: 'Tom'<br/>age: 25"]
        OP["Object.prototype<br/>━━━━━━━━━━<br/>toString: ƒ<br/>hasOwnProperty: ƒ"]
        NULL((null))

        Person -.->|"prototype 属性"| PP
        PP -.->|"constructor 属性"| Person
        P -->|"__proto__（步骤①设置）"| PP
        PP -->|"__proto__"| OP
        OP -->|"__proto__"| NULL
    end

    style A1 fill:#e6f7ff,stroke:#91d5ff
    style A2 fill:#bae7ff,stroke:#69c0ff
    style A3 fill:#91d5ff,stroke:#1890ff
    style Person fill:#1677ff,color:#fff
    style PP fill:#52c41a,color:#fff
    style P fill:#722ed1,color:#fff
    style OP fill:#faad14,color:#fff
    style NULL fill:#ff4d4f,color:#fff
```
...
| 连线 | 含义 | 由谁设置 |
|------|------|---------|
| `Person → Person.prototype`（虚线 `prototype`） | 构造函数通过 `prototype` 属性指向原型对象 | 函数定义时自动创建 |
| `Person.prototype → Person`（虚线 `constructor`） | 原型对象通过 `constructor` 指回构造函数 | 函数定义时自动创建 |
| `p → Person.prototype`（实线 `__proto__`） | 实例的原型指向构造函数的 `prototype` | **`new` 步骤①设置** |
| `Person.prototype → Object.prototype` | 原型对象本身也是普通对象，其原型是 `Object.prototype` | 对象创建时自动设置 |
| `Object.prototype → null` | 原型链终点 | 语言规范定义 |

> **核心记忆**：`new` 的本质就是"创建对象 → 挂原型 → 绑 this → 返回"。步骤①的"挂原型"操作使实例通过 `__proto__` 与构造函数的 `prototype` 建立联系，这就是原型链的起点。三条核心连线构成"构造函数 ↔ 原型对象 ← 实例"的三角关系：构造函数与原型对象双向关联（`prototype` / `constructor`），实例单向指向原型对象（`__proto__`）。
...
**原型链继承的特殊结构：当原型是另一个对象的实例**

`Child.prototype = new Parent()` 是原型链继承的核心语句。执行后，`Child.prototype` 不再是一个普通对象，而是 **Parent 构造函数的实例**——这形成了与普通 `new` 不同的特殊原型链结构。下图展示其形成过程与最终结构：

```mermaid
flowchart TB
    subgraph S1["关键代码执行过程"]
        direction TB
        C1["Parent 函数体: this.colors = ['red', 'blue']"]
        C2["new Parent() 执行:<br/>① 创建空对象 obj<br/>② obj.__proto__ = Parent.prototype<br/>③ Parent.call(obj) → obj.colors = ['red','blue']<br/>④ 返回 obj（携带 colors 属性）"]
        C3["Child.prototype = obj<br/>→ Child.prototype 现在是 Parent 实例"]
        C1 --> C2 --> C3
    end

    subgraph S2["形成的原型链结构（重点: Child.prototype 是 Parent 实例）"]
        Child["Child 构造函数"]
        CP["⚠️ Child.prototype<br/>════════════════<br/>这不是普通对象!<br/>这是 new Parent() 的实例<br/>════════════════<br/>自有属性(来自Parent构造函数):<br/>  colors: ['red', 'blue'] ← 引用类型!<br/>自有属性(手动修复):<br/>  constructor: Child"]
        Parent["Parent 构造函数"]
        PP["Parent.prototype<br/>━━━━━━━━━━<br/>say: ƒ"]
        OP["Object.prototype<br/>━━━━━━━━━━<br/>toString: ƒ"]
        NULL((null))

        c1["c1 实例 (new Child)<br/>━━━━━━<br/>无自有 colors"]
        c2["c2 实例 (new Child)<br/>━━━━━━<br/>无自有 colors"]
    end

    c1 -->|"__proto__"| CP
    c2 -->|"__proto__"| CP
    CP -->|"__proto__（CP是Parent实例）"| PP
    PP -->|"__proto__"| OP
    OP -->|"__proto__"| NULL
    ...
    style C1 fill:#fffbe6,stroke:#faad14
    style C2 fill:#fff1b8,stroke:#faad14
    style C3 fill:#ffe58f,stroke:#faad14
    style CP fill:#fa541c,color:#fff,stroke-width:3px
    style c1 fill:#722ed1,color:#fff
    style c2 fill:#722ed1,color:#fff
    style PP fill:#52c41a,color:#fff
    style Child fill:#1677ff,color:#fff
    style Parent fill:#1677ff,color:#fff
    style OP fill:#faad14,color:#fff
    style NULL fill:#ff4d4f,color:#fff
```
...
正常情况下（如上方 `new Person()` 的图示），构造函数的 `prototype` 是一个仅包含 `constructor` 和方法的普通对象。但 `Child.prototype = new Parent()` 后，`Child.prototype` 变成了 Parent 的实例，带来两个关键差异：

| 差异 | 普通原型对象 | 原型是实例（`new Parent()`） |
|------|------------|---------------------------|
| **原型上的属性** | 仅有 `constructor` + 方法定义 | **携带 Parent 的实例属性**（如 `colors`） |
| **原型链层级** | `实例 → Constructor.prototype → Object.prototype → null` | `实例 → Constructor.prototype → Parent.prototype → Object.prototype → null`（多出 `Parent.prototype` 一层） |
| **引用类型共享** | 无实例属性，不存在此问题 | ⚠️ **所有 Child 实例共享同一个 `colors` 数组** |

**引用类型共享问题详解**：

由于 `colors` 数组位于 `Child.prototype` 上（而非实例自身），所有 `Child` 实例通过原型链访问的是**同一个数组引用**：

```
c1.colors.push('green')
     ↓ 原型链查找
c1 自身无 colors → 沿 __proto__ 找到 Child.prototype.colors = ['red','blue']
     ↓ push('green') 修改的是原型上的数组（不是 c1 自身）
Child.prototype.colors 变为 ['red','blue','green']

c2.colors ← 同样沿原型链找到同一个数组 → ['red','blue','green']  ⚠️ 被污染了！
```

**与 `Object.create(Parent.prototype)` 的对比**：

```mermaid
flowchart LR
    subgraph A["方式A: new Parent() 原型链继承"]
        direction TB
        A_c["c1 实例"] -->|"__proto__"| A_CP["Child.prototype<br/>= new Parent() 实例<br/>⚠️ 携带 colors: ['red','blue']<br/>⚠️ 所有实例共享此数组"]
        A_CP -->|"__proto__"| A_PP["Parent.prototype<br/>say: ƒ"]
    end

    subgraph B["方式B: Object.create() 寄生组合继承"]
        direction TB
        B_c["c1 实例"] -->|"__proto__"| B_CP["Child.prototype<br/>= Object.create(...)<br/>✅ 无实例属性<br/>✅ 仅 constructor: Child"]
        B_CP -->|"__proto__"| B_PP["Parent.prototype<br/>say: ƒ"]
    end

    style A_CP fill:#fa541c,color:#fff,stroke-width:2px
    style B_CP fill:#52c41a,color:#fff,stroke-width:2px
```

> **核心区别**：`Object.create(Parent.prototype)` 创建的是一个**没有自有属性的纯净对象**，其 `__proto__` 同样指向 `Parent.prototype`，但**不执行 Parent 构造函数**，因此不携带 `colors` 等实例属性，从根本上避免了引用类型共享问题。两种方式的 `__proto__` 指向完全相同（都是 `Parent.prototype`），差别仅在于 `Child.prototype` 这个中间对象是否携带了 Parent 的实例属性——这正是寄生组合继承优于原型链继承的根本原因。
