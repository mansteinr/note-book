...
| **TTFB (Time to First Byte)** | 首字节到达时间 | ≤ 800ms（建议值） |

> **Core Web Vitals（核心网页指标）** 当前由 LCP、INP、CLS 组成；INP 已在 2024 年取代 FID。目标值应以真实用户数据的第 75 百分位（p75）为准，而不是只看本机 Lighthouse 分数。

### 1.2 优化方法论：先测量，再改动

性能优化不是“套技巧”，而是一个闭环：

1. **定义体验目标**：例如首屏 LCP、关键操作 INP、列表滚动帧率。
2. **区分数据来源**：Lighthouse/DevTools 是实验室数据；RUM（真实用户监控）才能反映网络、设备和地域差异。
3. **定位瓶颈**：Network 找传输和缓存，Performance 找长任务与布局，Coverage 找未使用代码，Memory 找泄漏。
4. **做最小改动并验证**：比较改动前后的 p75 指标、错误率和业务转化，避免“分数提高、体验变差”。
...
2. **按语义选择循环**：不要只为微小基准差异牺牲可读性；`for...in` 会枚举原型链属性，不应用于普通数组遍历
3. **避免在循环中操作 DOM**
...
```javascript
// 可读性优先：现代引擎通常会优化 arr.length；只有测量证明热点存在时才做微优化
for (let i = 0; i < arr.length; i++) {
  // ...
}

// 对稳定数组也可以缓存 length，避免循环体中数组长度被修改造成语义变化
...
```javascript
// 使用空闲时间处理低优先级任务；关键交互不能依赖它，因为浏览器可能长期没有空闲时间
...
**移动端性能优化要点**：
1. **动画优先使用 `transform` / `opacity`**：通常可避免布局，但仍需在目标设备实测
2. **正确设置 viewport**：现代移动浏览器在正确 viewport 下通常已无传统 300ms 点击延迟
...
> **实践提示**：HTTP/2 Server Push 已被主流浏览器移除或不再推荐。优先使用 `preload`、`103 Early Hints`、合理缓存和资源优先级来提前获取关键资源。
...
/* 优先动画 transform / opacity；不需要为了“硬件加速”强行添加 translateZ(0) */
.smooth {
...
它们通常可以避免布局；是否独立合成由浏览器决定，并非规范保证。`filter` 可能触发昂贵的绘制，`will-change` 只是提示且会增加内存占用，必须通过 DevTools 实测后少量使用。
...
**合成层要点**：浏览器会基于实现和当前页面状态决定分层；`<video>`、`<canvas>`、3D transform、`will-change` 等可能促成分层，但不存在可靠的“写某个 CSS 就一定 GPU 加速”的清单。
...
```javascript
// 使用 web-vitals 库（当前 Core Web Vitals 使用 INP，而非已废弃的 FID）
...
> **答**：① 多路复用：一个 TCP 连接并行多个请求；② 头部压缩：HPACK 算法；③ 二进制分帧；④ 流优先级机制。HTTP/2 仍受 TCP 层队头阻塞影响，HTTP/3 通过 QUIC 改善这一点。Server Push 已不建议作为优化手段，关键资源预加载优先考虑 `preload` 或 `103 Early Hints`。
...
> **答**：当前 Core Web Vitals 包括：① LCP（最大内容绘制）≤ 2.5s；② INP（交互到下一次绘制）≤ 200ms；③ CLS（累积布局偏移）≤ 0.1。评估应优先看真实用户数据 p75；FID 已被 INP 取代。
...
> **答**：① 性能指标采集：Navigation Timing、PerformanceObserver、`web-vitals`（FCP/LCP/INP/CLS/TTFB）；② 错误监控：`window.onerror`、`unhandledrejection`；③ 资源监控：PerformanceResourceTiming；④ 关联路由、设备、网络和业务操作；⑤ 用 `navigator.sendBeacon` 上报，并在看板按 p75、版本和地域观察趋势。优化前后应比较真实用户数据，而非只比较本机分数。
