...
**MutationObserver（DOM 树变化监听器）** 恰好解决上述问题：它能监听整个文档树的节点增删、属性变化、子树结构调整，配合稳定的 CSS Selector 或 `data-track-*` 属性约定，可在任何 DOM 出现的瞬间自动完成埋点绑定——这就是"自动埋点 / 零代码埋点"的技术基础。
...
| 阶段 | 内容 | 产出物 |
|------|-----|-------|
| 第 1 周 | 定 data-track-* 属性规范 + 埋点规则 DSL 评审 + 选型确认 | 规范文档、技术方案评审 |
| 第 2 周 | SDK 核心：EventQueue/Identity/RequestManager + MO 引擎骨架 + IO 曝光 | 单元测试通过 |
| 第 3 周 | 三个绑定通道（data-track/selector/attr-change）+ 去重 + 熔断机制 | Chrome 插件可视化规则生成初版 |
| 第 4 周 | Fastify 网关 + Kafka + ClickHouse 四张表联调，UV/订单误差 < 5% | 服务端压测报告 |
| 第 5 周 | 埋点规则平台：规则 CRUD、审批流、版本归档、灰度发布 | 平台可操作 Demo |
| 第 6 周 | 电商首页/列表/详情试点：data-track 属性改造 80% 页面 + 配置化 20% 规则 | 灰度 10% 上线 |
| 第 7 周 | Grafana 三块看板 + Prometheus 告警接入 + MO 性能压测（1000/s mutation） | 告警飞书闭环 |
| 第 8 周 | 灰度全量 + 埋点数据对比脚本（采样对比手动埋点，误差 < 3%） | 正式放量 + 文档交接 |
