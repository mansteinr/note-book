...
> 本文档系统梳理 Web Worker 的核心知识点与面试题，涵盖基本概念、使用场景、核心 API、通信机制、性能优化、浏览器兼容性及常见问题，题目类型包含选择题、简答题、编程题，难度覆盖基础、中级、高级三个层次，适合前端开发岗位面试复习使用。
...
- [二、面试题及详解](#二面试题及详解)
  - [题目 1：Web Worker 基本概念（选择题·基础）](#题目-1web-worker-基本概念选择题基础)
...
### 题目 1：Web Worker 基本概念（选择题·基础）

**难度**：基础　**类型**：选择题

**问题描述**：
以下关于 Web Worker 的说法，**错误**的是（　　）

A. Web Worker 运行在独立于主线程的后台线程中
B. Web Worker 可以直接访问主线程的 DOM 元素
C. Web Worker 与主线程之间通过 `postMessage` 进行通信
D. Web Worker 不能使用 `window` 和 `document` 对象

**参考答案**：**B**

**解析**：
- A 正确：Web Worker 的核心价值就是独立线程，避免阻塞 UI。
- B **错误**：Worker 中**不能直接操作 DOM**，也无法访问 `document`、`window`、`parent` 等与 UI 相关的对象。这是为了保证线程安全，避免多线程并发修改 DOM 造成的渲染冲突。
- C 正确：`postMessage` 是主线程与 Worker 的标准通信 API。
- D 正确：Worker 内的 `self` 指向 `WorkerGlobalScope`，而非 `window`，因此 `window` 和 `document` 不可用。

**评分标准**：选对得 2 分；选错 0 分。能否说出 B 错误的原因（DOM 限制）加 1 分。
...
**评分标准**：
- 每列举 1 个合理场景得 1 分，最高 4 分。
- 能归纳出使用 Worker 的判断标准（CPU 密集、不操作 DOM、通信收益大于开销）加 2 分。
- 满分 6 分。
...
**评分标准**：选对得 2 分；能列出至少 3 个 Worker 内不可用的 API 加 2 分。
