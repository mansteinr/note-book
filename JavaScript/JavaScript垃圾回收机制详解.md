...
| 内存释放 | `free` / `delete` | 自动（GC(垃圾回收) 判断） |
...
| **管理方式** | 自动管理，随作用域退出释放 | 由 GC(垃圾回收) 自动管理 |
...
> | 原文句子 | 到底是什么意思 | 通俗比喻 |
> |---------|--------------|---------|
> | **"将内存分为大小相等的两个空间（From 和 To）"** | 把一块连续内存**对半切**，一半叫 From（在用的那一半），另一半叫 To（备用、平时空闲）。两部分大小完全一样，所以总空间有 50% 是永远"浪费"的（备用）。 | 教室有两排座位，平时只让学生坐第一排，第二排空着备用。只有大扫除（GC）时才会用到。 |
> | **"只使用 From 空间分配对象"** | 平时程序创建新对象（`let obj = {}`）时，**只在 From 空间里给它分配内存**，To 空间完全不碰。这样一段时间后，From 空间里会装满对象，有的还活着、有的已经死了。 | 学生只能坐在第一排，第二排是空的不坐人。 |
> | **"GC 时将存活对象复制到 To 空间"** | 当 From 空间快满、触发 GC 时，从 GC Roots 出发做可达性检查，**找出 From 空间中仍然"活着"的对象，一个个按顺序复制到 To 空间里**。注意：已经死了（不可达）的对象**根本不复制**，直接丢弃。 | 大扫除时，让仍然需要的学生（存活对象）从第一排按顺序搬到第二排的新座位上；不需要的（死对象）就留下不搬了。 |
> | **"然后清空 From 空间"** | 所有存活对象都搬到 To 空间后，**整个 From 空间直接一整块被标记为"空闲"**（无需逐个擦除内容，效率极高）。原来 From 空间中死对象的数据会自然被后续覆盖。 | 第一排搬空后，直接把第一排整体标记成"干净的空座位"，不用逐个擦桌子。 |
> | **"交换两者角色"** | 最巧妙的一步：**原来的 From 变成下一轮的 To（备用），原来的 To 变成下一轮的 From（在用）**。这样就不用再把对象复制回 From，直接交换名字即可，零成本。 | 大扫除完后告诉大家："现在开始第二排是在用座位、第一排变成备用座位"——不用让学生坐回去，**直接换名字**就行，下次大扫除再反过来说。 |
>
> 🧮 **举一个具体小例子（数字级）**
>
